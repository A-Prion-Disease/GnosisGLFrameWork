...
Image ResourceSack::FindTexture(std::string TexName){

return Textures.at(TexName);
						    }

TextureSquare ResourceSack::FindTexSqr(std::string TexAtlas, int x, int y, int tileSize){

Image atlas = this->FindTexture(TexAtlas);

TextureSquare texSQR;

texSQR.LIT = atlas.width / tileSize;

texSQR.HIT = atlas.height / tileSize;
...
Quad::Quad(GLfloat x, GLfloat y, GLfloat z, GLfloat W, GLfloat H, GLfloat TexID, TextureSquare& TSQR){

const float Whole = 1.0f;

float TW = Whole/TSQR.LIT;
float TH = Whole/TSQR.HIT;

float X0 = TW * (TSQR.XSlot - 1);
float X1 = TW * TSQR.XSlot;

float Y0 = TH * (TSQR.HIT - TSQR.YSlot);
float Y1 = TH * (TSQR.HIT - TSQR.YSlot - 1);

GLfloat MixRatio = 0; //temp

Verts[0].Position = {x, y, z};
Verts[0].Color = {1.0f, 1.0f, 1.0f, 0.2f};
Verts[0].TextureCords = {X0, Y0};
Verts[0].TextureID = TexID;
Verts[0].mixRatio = MixRatio;
